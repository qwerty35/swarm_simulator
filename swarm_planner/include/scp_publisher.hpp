...
class TrajPublisherSCP {
public:
    TrajPublisherSCP(const ros::NodeHandle& _n,
                     std::shared_ptr<SwarmPlannerSCP> _swarm_planner,
            ...
            : n(_n),
              ...
              param(_param)
    {
        N = round(swarm_planner->msgs_traj_info.data[0]);
        K = round(swarm_planner->msgs_traj_info.data[1]);
        h = swarm_planner->msgs_traj_info.data[2];
        outdim = 3;
        ...
        p_curr.resize(N);
        build_mapping_mtx();
        std::vector<double> data = swarm_planner->msgs_traj_input.data;
...
private:
    ...
    void update_traj(double current_time){
        ...
        for(int qi = 0; qi < N; qi++) {
            ...
            transform.setOrigin(tf::Vector3(pos_des.pose.position.x, pos_des.pose.position.y, pos_des.pose.position.z));
            tf::Quaternion q;
            q.setRPY(0,0,0);
