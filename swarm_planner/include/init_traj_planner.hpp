...
namespace SwarmPlanning {
    ...
    public:
        ...
        InitTrajPlanner(std::shared_ptr<DynamicEDTOctomap> _distmap_obj,
            ...
            grid_z_min = ceil((param.world_z_min - SP_EPSILON) / param.grid_resolution) * param.grid_resolution;
            ...
            grid_z_max = floor((param.world_z_max + SP_EPSILON) / param.grid_resolution) * param.grid_resolution;

            dimx = (int) round((grid_x_max - grid_x_min) / param.grid_resolution) + 1;
            dimy = (int) round((grid_y_max - grid_y_min) / param.grid_resolution) + 1;
            dimz = (int) round((grid_z_max - grid_z_min) / param.grid_resolution) + 1;
        }
